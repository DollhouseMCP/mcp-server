...
describe('Portfolio Filtering Performance', () => {
  ...
  beforeEach(async () => {
    ...
    // Initialize manager directly (singleton was removed during refactor)
  ...
  describe('pattern matching performance', () => {
    it('should handle single pattern evaluation efficiently', () => {
      ...
      // Generous thresholds for CI environments (Windows, ARM emulation) which can be 3-5x slower
      expect(duration).toBeLessThan(5000); // Less than 5 seconds for CI variance
      expect(avgTimePerCall).toBeLessThan(0.5); // Less than 0.5ms per call for CI
