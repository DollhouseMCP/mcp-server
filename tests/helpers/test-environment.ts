 ...
 *
 * NOTE: For GitHub test credentials, use the centralized helpers from
 * src/config/test-env.ts instead of direct environment variable access.
 */
...
// Re-export these helpers for backward compatibility and convenience
// Tests importing test-environment.ts can also access test credential helpers
export { getTestGitHubToken, getTestGitHubUsername, getTestGitHubRepository } from '../../src/config/test-env.js';
...
export interface TestEnvironment {
  ...
  hasGitHubToken: boolean; // Use hasTestCredentials() from test-env.ts instead
...
export const getTestEnvironment = (): TestEnvironment => {
  return {
    ...
    hasGitHubToken: hasTestCredentials(), // Use centralized helper
