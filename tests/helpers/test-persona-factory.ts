...
/**
 * Create a test skill element
 */
export function createTestSkill(options: TestSkillOptions = {}): IElement {
  const prefix = options.prefix || 'test-qa-';
  const timestamp = Date.now();
  const name = options.name || `${prefix}test-skill-${timestamp}`;

  return {
    id: `skill_${name}_${timestamp}`,
    type: ElementType.SKILL,
    version: options.version || '1.0.0',
    metadata: {
      name,
      description: options.description || 'A test skill for QA validation',
      author: options.author || 'qa-tester',
      created: new Date().toISOString(),
      updated: new Date().toISOString(),
      tags: ['test', 'qa', 'automated'],
    ...
    validate: () => ({ isValid: true, errors: [] }),
    serialize: () => createSkillContent(name, options),
    deserialize: () => {},
    getStatus: () => 'inactive' as any
  };
}
 ...
 */
function createSkillContent(name: string, options: TestSkillOptions = {}): string {
  return `---
name: ${name}
description: ${options.description || 'A test skill for QA validation'}
author: ${options.author || 'qa-tester'}
...
This is a test skill created for QA validation.

## Purpose

This skill validates the DollhouseMCP portfolio sync functionality for skill elements.
