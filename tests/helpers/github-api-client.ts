...
// Import for documentation - this module receives credentials via TestEnvironment interface
// eslint-disable-next-line @typescript-eslint/no-unused-vars
import { getTestGitHubToken } from '../../src/config/test-env.js';
...
export class GitHubTestClient {
   ...
   *
   * Uses the repository name from the test environment configuration.
   * PortfolioRepoManager checks process.env.TEST_GITHUB_REPO, so we use the same repository.
   */
  ...
  async verifyUrl(url: string): Promise<boolean> {
    try {
      ...
      // For GitHub URLs, we can check via API or web
      if (parsedUrl.hostname === 'github.com' || parsedUrl.hostname === 'www.github.com') {
        // Check for blob URLs (direct file links)
        const blobMatch = url.match(/github\.com\/[^/]+\/[^/]+\/blob\/[^/]+\/(.+)/);
        if (blobMatch) {
          const file = await this.getFile(blobMatch[1]);
          return file !== null;
        }
        ...
        if (commitMatch) {
          ...
          // Verify commit exists via API
          const commitUrl = `${this.baseUrl}/repos/${owner}/${repo}/commits/${sha}`;
          const response = await this.fetchWithRetry(commitUrl, {
            headers: this.headers
          });
          return response.status === 200;
        }

        // Check for tree/branch URLs
        const treeMatch = url.match(/github\.com\/([^/]+)\/([^/]+)\/tree\//);
        if (treeMatch) {
          const [, owner, repo] = treeMatch;
          // Verify repository exists
          const repoUrl = `${this.baseUrl}/repos/${owner}/${repo}`;
          const response = await this.fetchWithRetry(repoUrl, {
            headers: this.headers
          });
          return response.status === 200;
        }
