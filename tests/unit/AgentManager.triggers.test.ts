...
import { createRealAgentManager } from '../helpers/di-mocks.js';
...
describe('AgentManager - Trigger Extraction', () => {
  ...
  beforeEach(async () => {
    ...
    // Create PortfolioManager instance with FileOperationsService
    ...
    // Create agent manager using factory function
    agentManager = createRealAgentManager(tempDir, tempDir, {
      portfolioManager
    });
...
Agent with mixed triggers.`;
      ...
      expect(loggerWarnMock).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          elementName: 'Agent With Mixed Triggers',
...
Agent with many triggers.`;
      ...
      expect(loggerWarnMock).toHaveBeenCalledWith(
        expect.stringContaining('Trigger limit exceeded (25 > 20)'),
        expect.objectContaining({
          elementName: 'Agent With Many Triggers',
          elementType: ElementType.AGENT,
          providedCount: 25,
          limit: 20,
          truncated: 5
...
Testing empty triggers.`;
      ...
      expect(loggerWarnMock).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          elementName: 'Agent With Empty Triggers',
          elementType: ElementType.AGENT,
          rejectedTriggers: expect.arrayContaining([
