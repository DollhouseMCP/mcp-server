...
describe('PortfolioSyncComparer', () => {
  ...
  describe('compareElements', () => {
    const createLocalElement = (name: string, sha?: string) => ({
      name,
    ...
    const createRemoteElement = (name: string, sha?: string) => ({
      name,
      type: ElementType.PERSONA,
      path: `personas/${name}.md`,
      sha: sha || `sha-${name}`,
    ...
    describe('performance', () => {
      it('should handle large element lists efficiently', () => {
        ...
        // Use high-resolution performance timing for accuracy
        const startTime = performance.now();
        const result = comparer.compareElements(remoteElements, localElements, 'mirror');
        const endTime = performance.now();
        const duration = endTime - startTime;

        // Platform and Node version-specific thresholds
        // CI environments are significantly slower than local development
        // Node 20.x is 10-15% slower than Node 22.x on Map operations
        const nodeMajor = Number.parseInt(process.version.split('.')[0].substring(1));
        const nodeMultiplier = nodeMajor < 22 ? 1.2 : 1.0;

        let threshold = 100; // Local development baseline
        if (process.env.CI) {
          switch (process.platform) {
            case 'win32':
              threshold = Math.floor(200 * nodeMultiplier); // 240ms for Node 20.x
              break;
            case 'darwin':
              threshold = Math.floor(150 * nodeMultiplier); // 180ms for Node 20.x
              break;
            case 'linux':
              threshold = Math.floor(130 * nodeMultiplier); // 156ms for Node 20.x
              break;
            default:
              threshold = Math.floor(200 * nodeMultiplier);
          }
        }
