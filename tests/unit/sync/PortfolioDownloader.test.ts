...
Content with émojis 🎉`;
  ...
  describe('downloadBatch', () => {
    ...
    it('should download all files with progress tracking', async () => {
      const files = Array.from({ length: 5 }, (_, i) => `personas/file${i}.md`);
      ...
      const onProgress = jest.fn<(downloaded: number, total: number) => void>();
