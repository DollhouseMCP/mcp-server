...
const ROUTING_CHANGES: ReadonlyArray<readonly [string, Partial<IntegrationDescriptorRecord>]> = [
  ...
  ['OAuth configuration', {
    oauth: { ...requiredOAuthConfig(), tokenUrl: 'https://accounts.example/oauth/v2/token' },
  }],
...
function requiredOAuthConfig(): NonNullable<IntegrationDescriptorRecord['oauth']> {
  const oauth = descriptor().oauth;
  if (!oauth) throw new Error('Expected descriptor OAuth fixture');
  return oauth;
}
...
describe('integrationDescriptorRoutingFingerprint', () => {
  ...
  it('rejects routing values that JSON.stringify cannot encode', () => {
    const nonJsonPromotion = {
      enabled: Symbol('non-json-routing-value'),
    } as unknown as IntegrationDescriptorRecord['operationPromotion'];

    expect(() => integrationDescriptorRoutingFingerprint(descriptor({
      operationPromotion: nonJsonPromotion,
    }))).toThrow('descriptor routing values must be JSON-compatible');
  });
});
