...
describe('deriveIntegrationClientSecretRevision', () => {
  ...
  it('is stable for the same provider and secret', () => {
    expect(deriveIntegrationClientSecretRevision(hasher, 'examplecorp', 'client-secret'))
      .toBe(deriveIntegrationClientSecretRevision(hasher, 'examplecorp', 'client-secret'));
  });

  it('changes when the provider or secret changes', () => {
    const revision = deriveIntegrationClientSecretRevision(hasher, 'examplecorp', 'client-secret');
    expect(deriveIntegrationClientSecretRevision(hasher, 'examplecorp', 'rotated-secret'))
      .not.toBe(revision);
    expect(deriveIntegrationClientSecretRevision(hasher, 'othercorp', 'client-secret'))
      .not.toBe(revision);
  });
