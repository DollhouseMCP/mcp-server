...
const ALLOWED_EMAIL = 'alice@example.test';
...
describe('ConsoleAccountAllowlistSignInAuthority', () => {
  it('uses active console account allowlist entries as the sign-in authority', async () => {
    ...
    await expect(authority.matchesIdentity({ email: ALLOWED_EMAIL })).resolves.toBe(false);
    await expect(authority.deniesIdentity({ email: ALLOWED_EMAIL })).resolves.toBe(false);
    ...
    await expect(authority.matchesIdentity({ email: ALLOWED_EMAIL })).resolves.toBe(true);
    ...
    await expect(authority.matchesIdentity({ email: ALLOWED_EMAIL })).resolves.toBe(false);
    await expect(authority.deniesIdentity({ email: ALLOWED_EMAIL })).resolves.toBe(true);
