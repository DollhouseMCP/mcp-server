...
// Create mock TokenManager instance
const mockTokenManagerInstance = {
  getGitHubTokenAsync: jest.fn(() => Promise.resolve('test-token')),
  getGitHubToken: jest.fn().mockReturnValue('test-token'),
  validateTokenScopes: jest.fn(() => Promise.resolve({
    isValid: true,
    scopes: ['public_repo']
  })),
...
describe('PortfolioRepoManager', () => {
  ...
  beforeEach(async () => {
    jest.clearAllMocks();
    // Clear GitHub repo env var for clean test state
    delete process.env.GITHUB_REPOSITORY;

    // Reset TokenManager mock
    mockTokenManagerInstance.getGitHubTokenAsync.mockResolvedValue('test-token');
    mockTokenManagerInstance.validateTokenScopes.mockResolvedValue({
      isValid: true,
      scopes: ['public_repo']
    });
    ...
    // Create manager with TokenManager instance (TokenManager is now required first param)
    manager = new PortfolioRepoManager(mockTokenManagerInstance as any);
  });
  ...
  describe('repository name configuration', () => {
    it('should use provided repository name parameter', () => {
      const customManager = new PortfolioRepoManager(mockTokenManagerInstance as any, 'custom-portfolio');
      expect(customManager.getRepositoryName()).toBe('custom-portfolio');
    });

    it('should fall back to GITHUB_REPOSITORY when no parameter provided', () => {
      process.env.GITHUB_REPOSITORY = 'env-portfolio';
      const envManager = new PortfolioRepoManager(mockTokenManagerInstance as any);
      expect(envManager.getRepositoryName()).toBe('env-portfolio');
    });

    it('should fall back to default when no parameter and no env var', () => {
      delete process.env.GITHUB_REPOSITORY;
      const defaultManager = new PortfolioRepoManager(mockTokenManagerInstance as any);
      expect(defaultManager.getRepositoryName()).toBe('dollhouse-portfolio');
    });

    it('should prioritize constructor parameter over env var', () => {
      process.env.GITHUB_REPOSITORY = 'env-portfolio';
      const paramManager = new PortfolioRepoManager(mockTokenManagerInstance as any, 'param-portfolio');
      expect(paramManager.getRepositoryName()).toBe('param-portfolio');
    });

    it('should use configured repository name in API calls', async () => {
      const customManager = new PortfolioRepoManager(mockTokenManagerInstance as any, 'my-custom-repo');
