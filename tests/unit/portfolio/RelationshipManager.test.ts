...
import { EnhancedIndex } from '../../../src/portfolio/EnhancedIndexManager.js';
...
describe('RelationshipManager', () => {
  ...
  let mockIndexConfigManager: jest.Mocked<IndexConfigManager>;
  let mockVerbTriggerManager: jest.Mocked<VerbTriggerManager>;
  let mockNlpScoring: jest.Mocked<NLPScoringManager>;
  ...
  beforeEach(async () => {
    ...
    mockIndexConfigManager = {
      getConfig: jest.fn().mockReturnValue({}),
      updateConfig: jest.fn(),
    } as any;

    mockVerbTriggerManager = {
      getVerbsForElement: jest.fn().mockReturnValue([]),
      getElementsForVerb: jest.fn().mockReturnValue([]),
      getVerbCategory: jest.fn().mockReturnValue('general'),
    } as any;

    mockNlpScoring = {
      calculateSimilarity: jest.fn().mockReturnValue(0.5),
      scoreTokens: jest.fn().mockReturnValue({ score: 0.5, tokens: [] }),
    } as any;
    ...
    manager = new RelationshipManager({
      ...
      indexConfigManager: mockIndexConfigManager,
      verbTriggerManager: mockVerbTriggerManager,
      nlpScoring: mockNlpScoring
    });
  });
