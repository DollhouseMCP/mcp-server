...
import { UnifiedIndexManager, DuplicateInfo } from '../../../src/portfolio/UnifiedIndexManager.js';
...
describe('UnifiedIndexManager', () => {
  ...
  beforeEach(() => {
    ...
    mockPerformanceMonitor = {
      ...
      recordCachePerformance: jest.fn(),
      getSearchStats: jest.fn().mockReturnValue({ averageTime: 0, cacheHitRate: 0 }),
    ...
    // Create unified manager with DI
    unifiedManager = new UnifiedIndexManager({
      portfolioIndexManager: mockLocalIndexManager,
      githubIndexer: mockGitHubIndexer,
      collectionIndexCache: mockCollectionIndexCache,
  ...
  describe('Instance Creation', () => {
    it('should create separate instances', () => {
      const instance1 = new UnifiedIndexManager({
        portfolioIndexManager: mockLocalIndexManager,
        githubIndexer: mockGitHubIndexer,
        performanceMonitor: mockPerformanceMonitor
      });
      const instance2 = new UnifiedIndexManager({
        portfolioIndexManager: mockLocalIndexManager,
        githubIndexer: mockGitHubIndexer,
        performanceMonitor: mockPerformanceMonitor
      });
  ...
  describe('Source Priority (Issue #1446)', () => {
    ...
    describe('getVersionComparison()', () => {
      it('should check all sources for version information', async () => {
        // Mock checkDuplicates to return duplicate info
        ...
        // Spy on checkDuplicates to return our mock data
        jest.spyOn(unifiedManager, 'checkDuplicates').mockResolvedValue([duplicateInfo]);

        const versionInfo = await unifiedManager.getVersionComparison('Test Persona');
      ...
      it('should return null when element not found', async () => {
        // Spy on checkDuplicates to return empty array (no duplicates found)
        jest.spyOn(unifiedManager, 'checkDuplicates').mockResolvedValue([]);

        const versionInfo = await unifiedManager.getVersionComparison('Nonexistent Element');
