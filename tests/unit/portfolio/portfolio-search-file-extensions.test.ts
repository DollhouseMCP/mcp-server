...
describe('Portfolio Search File Extension Display (Issue #1213)', () => {
  ...
  beforeEach(async () => {
    ...
    testPortfolioDir = path.join(tmpdir(), 'dollhouse', 'portfolio-test-' + Date.now());
    process.env.DOLLHOUSE_PORTFOLIO_DIR = testPortfolioDir;

    // Initialize container which will respect the env var for portfolio path
    container = new DollhouseContainer();
    portfolioManager = container.resolve<PortfolioManager>('PortfolioManager');
    indexManager = container.resolve<PortfolioIndexManager>('PortfolioIndexManager');

    // Initialize the portfolio directory structure for the test
    await portfolioManager.initialize();
  });

  afterEach(async () => {
    // Cleanup container and its services
