...
describe('PersonaElement', () => {
  describe('constructor', () => {
    it('should create a PersonaElement with default metadata', () => {
      ...
      const persona = new PersonaElement(metadata, 'Test content', 'sample.md', metadataService);
      ...
      expect(persona.instructions).toBe('Test content');
    ...
    it('should generate an ID based on name', () => {
      ...
      const persona = new PersonaElement(metadata, '', '', metadataService);
  ...
  describe('legacy conversion', () => {
    it('should convert from legacy Persona interface', () => {
      ...
      const persona = PersonaElement.fromLegacy(legacyPersona, metadataService);
      ...
      expect(persona.metadata.version).toBe('2.1.0');  // MetadataService normalizes version to semver
      ...
      expect(persona.instructions).toBe('Legacy persona content');
    ...
    it('should convert to legacy Persona interface', () => {
      ...
      const persona = new PersonaElement(metadata, 'Modern content', 'modern.md', metadataService);
  ...
  describe('validation', () => {
    it('should validate a complete persona', () => {
      ...
      const persona = new PersonaElement(metadata, 'Valid persona content', '', metadataService);
    ...
    it('should detect empty instructions', () => {
      const metadata: Partial<PersonaElementMetadata> = {
        name: 'Empty Persona',
        description: 'A persona with no instructions'
      };
      ...
      expect(result.errors![0].field).toBe('instructions');
      expect(result.errors![0].message).toBe('Persona instructions cannot be empty');
      expect(result.errors![0].code).toBe('EMPTY_INSTRUCTIONS');
    });

    it('should warn about long instructions', () => {
      const metadata: Partial<PersonaElementMetadata> = {
        name: 'Long Persona',
        description: 'A persona with very long instructions'
      };

      const longContent = 'x'.repeat(15000);
      const persona = new PersonaElement(metadata, longContent, '', metadataService);
      const result = persona.validate();

      expect(result.valid).toBe(true);
      expect(result.warnings).toHaveLength(1);
      expect(result.warnings![0].field).toBe('instructions');
      expect(result.warnings![0].message).toBe('Persona instructions are very long, consider breaking them down');
    ...
    it('should warn about many triggers', () => {
      ...
      const persona = new PersonaElement(metadata, 'Content with many triggers', '', metadataService);
      const result = persona.validate();
    ...
    it('should warn about missing adult flag for 18+ content', () => {
      ...
      const persona = new PersonaElement(metadata, 'Adult content', '', metadataService);
      const result = persona.validate();
  ...
  describe('serialization', () => {
    it('should serialize to markdown format', () => {
      ...
      const persona = new PersonaElement(metadata, 'Serializable content', '', metadataService);
...
This is the deserializable content.`;
      ...
      const persona = new PersonaElement({}, '', '', metadataService);
      ...
      expect(persona.instructions).toBe('This is the deserializable content.');
    });
    ...
    it('should handle round-trip serialization', () => {
      ...
      const original = new PersonaElement(metadata, 'Round-trip content', 'roundtrip.md', metadataService);
      const serialized = original.serialize();

      const restored = new PersonaElement({}, '', '', metadataService);
  ...
  describe('lifecycle', () => {
    it('should handle activation', async () => {
      ...
      const persona = new PersonaElement(metadata, 'Activatable content', '', metadataService);
    ...
    it('should handle deactivation', async () => {
      ...
      const persona = new PersonaElement(metadata, 'Deactivatable content', '', metadataService);
  ...
  describe('feedback processing', () => {
    it('should process feedback if method is available', () => {
      ...
      const persona = new PersonaElement(metadata, 'Feedback content', '', metadataService);
