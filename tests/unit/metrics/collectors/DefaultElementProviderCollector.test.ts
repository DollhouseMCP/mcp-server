...
describe('DefaultElementProviderCollector', () => {
  ...
  describe('metric types', () => {
    test('hits_total is a counter', () => {
      const collector = new DefaultElementProviderCollector(makeMockStatsFn());
      const metrics = collector.collect() as MetricEntry[];
      const entry = findByName(metrics, 'portfolio.buffer_pool.hits_total') as CounterEntry;
    ...
    test('misses_total is a counter', () => {
      const collector = new DefaultElementProviderCollector(makeMockStatsFn());
      const metrics = collector.collect() as MetricEntry[];
      const entry = findByName(metrics, 'portfolio.buffer_pool.misses_total') as CounterEntry;
      expect(entry).toBeDefined();
      expect(entry.type).toBe('counter');
    });
    ...
    test('created_total is a counter', () => {
      const collector = new DefaultElementProviderCollector(makeMockStatsFn());
      const metrics = collector.collect() as MetricEntry[];
      const entry = findByName(metrics, 'portfolio.buffer_pool.created_total') as CounterEntry;
      expect(entry).toBeDefined();
      expect(entry.type).toBe('counter');
    });
    ...
    test('hit_rate is a gauge', () => {
      const collector = new DefaultElementProviderCollector(makeMockStatsFn());
      const metrics = collector.collect() as MetricEntry[];
      const entry = findByName(metrics, 'portfolio.buffer_pool.hit_rate') as GaugeEntry;
      expect(entry).toBeDefined();
    ...
    test('size_current is a gauge', () => {
      const collector = new DefaultElementProviderCollector(makeMockStatsFn());
      const metrics = collector.collect() as MetricEntry[];
      const entry = findByName(metrics, 'portfolio.buffer_pool.size_current') as GaugeEntry;
      expect(entry).toBeDefined();
      expect(entry.type).toBe('gauge');
    });
    ...
    test('size_max is a gauge', () => {
      const collector = new DefaultElementProviderCollector(makeMockStatsFn());
      const metrics = collector.collect() as MetricEntry[];
      const entry = findByName(metrics, 'portfolio.buffer_pool.size_max') as GaugeEntry;
      expect(entry).toBeDefined();
      expect(entry.type).toBe('gauge');
    });
  });
  ...
  describe('statsFn injection', () => {
    test('accepts an injected statsFn and uses its return values', () => {
      ...
      const metrics = collector.collect() as MetricEntry[];
      const hitsEntry = findByName(metrics, 'portfolio.buffer_pool.hits_total') as CounterEntry;
