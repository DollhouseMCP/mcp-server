...
// Helper function to create unique random bytes for request IDs
...
describe('GitHubRateLimiter', () => {
  let rateLimiter: InstanceType<typeof GitHubRateLimiter>;
  let hadStatusCheckInterval: boolean;
  ...
  beforeEach(() => {
    ...
    // GitHubRateLimiter needs a TokenManager instance
    rateLimiter = new GitHubRateLimiter(mockTokenManagerInstance as any);

    // Save whether interval was set, then clear it to prevent infinite timer loops in tests
    hadStatusCheckInterval = !!(rateLimiter as any).statusCheckInterval;
    if ((rateLimiter as any).statusCheckInterval) {
      clearInterval((rateLimiter as any).statusCheckInterval);
      (rateLimiter as any).statusCheckInterval = undefined;
    }
  });
  ...
  describe('Constructor behavior', () => {
    ...
    it('should set up periodic check timer', () => {
      // Verify that the rate limiter had set up an interval (we saved this before clearing it)
      expect(hadStatusCheckInterval).toBe(true);
    });
  ...
  describe('Lazy initialization', () => {
    ...
    it('should only initialize once even with multiple concurrent requests', async () => {
      // Mock randomBytes to prevent periodic auth updates during queue processing
      mockRandomBytes.mockImplementation((size: number) => {
        if (size === 6) {
          return Buffer.from('1234567890ab', 'hex');
        } else if (size === 1) {
          return Buffer.from([100]); // > 26, so shouldUpdate is false
        }
        return Buffer.alloc(size);
      });
      ...
      // Allow microtasks to run and advance timers
  ...
  describe('Error recovery', () => {
    it('should continue with defaults if initialization fails', async () => {
      // Mock randomBytes to prevent periodic auth updates
      mockRandomBytes.mockImplementation((size: number) => {
        if (size === 6) {
          return Buffer.from('1234567890ab', 'hex');
        } else if (size === 1) {
      ...
      // Should have logged the error at both levels
      expect(mockLogger.warn).toHaveBeenCalledWith(
        'Failed to check authentication status for rate limiting',
      ...
      // API call should still have been made (with fallback defaults)
