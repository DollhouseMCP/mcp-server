...
- Item 3: Third achievement`;
  ...
  describe('Type Validation', () => {
    ...
    it('should validate render() return type', async () => {
      ...
      mockTemplate.render = jest.fn<() => Promise<any>>().mockResolvedValue(123);
  ...
  describe('Error Handling', () => {
    // FIX: Issue #275 - TemplateRenderer now throws ElementNotFoundError for missing templates
    it('should handle template not found', async () => {
      // PROOF: Missing templates throw ElementNotFoundError
      mockTemplateManager.find.mockResolvedValue(undefined);

      // VERIFICATION: ElementNotFoundError is thrown for missing templates
      await expect(renderer.render('nonexistent')).rejects.toThrow(ElementNotFoundError);
    });
    ...
    it('should handle render() exceptions', async () => {
      ...
      mockTemplate.render = jest.fn<() => Promise<string>>().mockRejectedValue(new Error('Render failed'));
  ...
  describe('Section-format rendering (issue #705)', () => {
    const SECTION_CONTENT = [
      ...
      '</style>',
      '<script>',
      'function greet() { return { ok: true }; }',
      '</script>',
    ].join('\n');

    function makeSectionTemplate(name: string): Template {
      const t = new Template(
        { name, variables: [{ name: 'name', type: 'string' }] },
        SECTION_CONTENT,
        metadataService
      );
      return t;
    ...
    it('should render the <template> section with variable substitution by default', async () => {
      const mockTemplate = makeSectionTemplate('section-tmpl');
      mockTemplateManager.find.mockResolvedValue(mockTemplate);

      const result = await renderer.render('section-tmpl', { name: 'Alice' });

      expect(result.success).toBe(true);
      expect(result.content!.trim()).toBe('Hello Alice!');
    });

    it('should return raw <style> section without variable substitution', async () => {
      const mockTemplate = makeSectionTemplate('section-tmpl');
      mockTemplateManager.find.mockResolvedValue(mockTemplate);

      const result = await renderer.render('section-tmpl', {}, 'style');

      expect(result.success).toBe(true);
      expect(result.content).toContain('.card { padding: 0; }');
    });

    it('should return raw <script> section without variable substitution', async () => {
      const mockTemplate = makeSectionTemplate('section-tmpl');
      mockTemplateManager.find.mockResolvedValue(mockTemplate);

      const result = await renderer.render('section-tmpl', {}, 'script');

      expect(result.success).toBe(true);
      expect(result.content).toContain('function greet()');
      // }} in script should not be treated as a template token
      expect(result.content).toContain('{ return { ok: true }; }');
    });

    it('should return all sections when allSections is true', async () => {
      const mockTemplate = makeSectionTemplate('section-tmpl');
      mockTemplateManager.find.mockResolvedValue(mockTemplate);

      const result = await renderer.render('section-tmpl', { name: 'Bob' }, undefined, true);

      expect(result.success).toBe(true);
      expect(result.sections).toBeDefined();
      expect(result.sections!.template.trim()).toBe('Hello Bob!');
      expect(result.sections!.style).toContain('.card');
      expect(result.sections!.script).toContain('function greet');
    });

    it('should error when requesting style section from a non-section template', async () => {
      const plain = new Template({ name: 'plain' }, 'Hello {{name}}!', metadataService);
      mockTemplateManager.find.mockResolvedValue(plain);

      const result = await renderer.render('plain', {}, 'style');
