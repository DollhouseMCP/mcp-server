...
describe('TemplateRenderer - Unicode Security', () => {
  ...
  describe('Error Messages with Unicode', () => {
    ...
    it('should use normalized name in error messages', async () => {
      ...
      // Error should reference the normalized name (ASCII 'template')
      await expect(renderer.render(nameWithUnicode, {})).rejects.toThrow(ElementNotFoundError);

      // Verify the error contains the normalized name
      try {
        await renderer.render(nameWithUnicode, {});
      } catch (error) {
        expect(error).toBeInstanceOf(ElementNotFoundError);
        expect((error as ElementNotFoundError).message).toContain('template'); // Normalized to ASCII
        expect((error as ElementNotFoundError).message).toContain('not found');
      }
    });
  });
});
