...
let configManager: InstanceType<typeof ConfigModule.ConfigManager>;
  ...
  beforeEach(async () => {
    ...
    // Create mock FileOperationsService that wraps the fs mocks
    // ConfigManager now expects IFileOperationsService, not fs directly
    const mockFileOperations = {
      readFile: jest.fn().mockImplementation((path: string) => mockReadFile(path, 'utf-8')),
      writeFile: jest.fn().mockImplementation((path: string, content: string) => mockWriteFile(path, content, 'utf-8')),
      exists: jest.fn().mockResolvedValue(false), // No backup exists initially
      createDirectory: jest.fn().mockImplementation((dir: string) => mockMkdir(dir, { recursive: true })),
