...
describe('CollectionIndexManager - Essential Tests', () => {
  let manager: InstanceType<typeof CollectionIndexManager>;
  ...
  let mockFileOperationsService: ReturnType<typeof createMockFileOperationsService>;
  ...
  beforeEach(async () => {
    ...
    mockFileOperationsService = createMockFileOperationsService();
    ...
    container.register('CollectionIndexManager', () => new CollectionIndexManager({
      ...
      fileOperations: mockFileOperationsService
    }));
    manager = container.resolve('CollectionIndexManager');
    sleepSpy = jest
      .spyOn(CollectionIndexManager.prototype as any, 'sleep')
  ...
  afterEach(async () => {
    ...
    if (manager) {
      await manager.waitForBackgroundRefresh();
      await manager.clearCache();
    }
    sleepSpy?.mockRestore();
    jest.clearAllMocks();
  ...
  describe('constructor', () => {
    test('should initialize with default configuration', () => {
      container.register('DefaultCollectionIndexManager', () => new CollectionIndexManager({ fileOperations: mockFileOperationsService }));
      const defaultManager: InstanceType<typeof CollectionIndexManager> = container.resolve('DefaultCollectionIndexManager');
    ...
    test('should use environment variable for fetch timeout', () => {
      ...
      container.register('EnvCollectionIndexManager', () => new CollectionIndexManager({ fileOperations: mockFileOperationsService }));
      const managerWithEnv: InstanceType<typeof CollectionIndexManager> = container.resolve('EnvCollectionIndexManager');
    ...
    test('should ignore invalid environment variable', () => {
      ...
      container.register('BadEnvCollectionIndexManager', () => new CollectionIndexManager({ fileOperations: mockFileOperationsService }));
      const managerWithBadEnv: InstanceType<typeof CollectionIndexManager> = container.resolve('BadEnvCollectionIndexManager');
      delete process.env.COLLECTION_FETCH_TIMEOUT;
