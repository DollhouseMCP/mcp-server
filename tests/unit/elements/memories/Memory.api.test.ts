...
describe('Memory API Integration (Issue #1320)', () => {
  ...
  beforeEach(async () => {
    ...
    tempPortfolioDir = await fs.mkdtemp(path.join(os.tmpdir(), 'memory-api-test-'));
  ...
  afterEach(async () => {
    ...
    // FIX: Properly dispose of DI container to clean up async resources
    // (PerformanceMonitor, file watchers, event listeners, etc.)
  ...
  describe('End-to-end workflow', () => {
    it('should support full validation workflow', async () => {
      ...
      const memory = new Memory({ name: 'Workflow Test API' }, metadataService);
  ...
  describe('Integration with MemoryManager', () => {
    it('should have file path set after loading', async () => {
      ...
      const memory = new Memory({ name: 'Load Test API Memory' }, metadataService);
      await memory.addEntry('Test content', ['test']);
    ...
    it('should allow save without explicit path after load', async () => {
      // Create and save
      const memory = new Memory({ name: 'Auto Save Test API Memory' }, metadataService);
