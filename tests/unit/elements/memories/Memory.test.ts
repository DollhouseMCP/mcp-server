...
describe('Memory Element', () => {
  ...
  describe('Constructor', () => {
    it('should create a memory with default values', () => {
      const defaultMemory = new Memory({}, metadataService);
      expect(defaultMemory.type).toBe(ElementType.MEMORY);
      expect(defaultMemory.metadata.name).toBe('Unnamed Memory');
      expect((defaultMemory.metadata as any).storageBackend).toBe('memory');
      // Memories are permanent by default (999999 days ≈ 2739 years)
      expect((defaultMemory.metadata as any).retentionDays).toBe(999999);
      expect((defaultMemory.metadata as any).privacyLevel).toBe('private');
    });
  ...
  describe('search', () => {
    ...
    it('should respect privacy levels', async () => {
      ...
      const memory = new Memory({ privacyLevel: 'private' }, metadataService);
  ...
  describe('enforceRetentionPolicy', () => {
    it('should remove expired entries', async () => {
      ...
      const shortMemory = new Memory({ retentionDays: 1 }, metadataService);
  ...
  describe('getEntry and deleteEntry', () => {
    ...
    it('should delete entry by ID', async () => {
      ...
      const retrieved = await memory.getEntry(added.id);
      expect(retrieved).toBeUndefined(); // LRUCache.get() returns undefined for missing keys, like Map
    });
  ...
  describe('serialize and deserialize', () => {
    it('should serialize and deserialize correctly', async () => {
      ...
      const newMemory = new Memory({}, metadataService);
  ...
  describe('validate', () => {
    ...
    it('should validate max entries', () => {
      const invalidMemory = new Memory({ maxEntries: 5000 }, metadataService);
      const result = invalidMemory.validate();
  ...
  describe('Content Getter', () => {
    it('should format content correctly via content getter', async () => {
      const memory = new Memory({ name: 'Test Memory' }, metadataService);
