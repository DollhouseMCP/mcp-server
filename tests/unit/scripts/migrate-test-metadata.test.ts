...
function determineTestSuite(filePath: string): string {
  if (filePath.includes('tests/fixtures')) return 'test-fixtures';
  if (filePath.includes('test-elements')) return 'roundtrip-testing';
  if (filePath.includes('data/')) return 'bundled-test-data';
  if (filePath.includes('tests/integration')) return 'integration-testing';
  return 'unit-testing';
}
...
function isTestFile(filePath: string, content: string): boolean {
  ...
  if (relativePath.includes('tests/') || relativePath.includes('data/') || relativePath.includes('test-elements/')) {
    return true;
  }
...
This is a test file without frontmatter.`;
      ...
      const result = addTestMetadata(content, '/tests/fixtures/sample-test.md');
    ...
    it('should determine correct test suite based on path', () => {
      const testCases = [
        { path: '/tests/fixtures/sample.md', expectedSuite: 'test-fixtures' },
        ...
        { path: '/tests/integration/integration.md', expectedSuite: 'integration-testing' },
    ...
    it('should determine correct purpose based on filename and content', () => {
      ...
      for (const testCase of testCases) {
        const result = addTestMetadata(testCase.content, `/tests/${testCase.file}`);
    ...
    it('should include timestamp and version information', () => {
      const content = '# Test';
      const result = addTestMetadata(content, '/tests/file.md');
...
# Content`;
  ...
  describe('isTestFile', () => {
    it('should identify test files by directory path', () => {
      const testCases = [
        { path: '/project/tests/fixtures/file.md', content: '# Content', expected: true },
  ...
  describe('Migration Integration', () => {
    beforeEach(async () => {
      ...
      const testDirs = ['tests/fixtures', 'data/personas', 'test-elements'];
    ...
    it('should perform dry-run migration without modifying files', async () => {
      ...
      const testFiles = [
        {
          path: 'tests/fixtures/sample-persona.md',
          content: `---
name: Sample Persona
type: persona
---
...
# Test Data`
      ...
      const _originalCwd = process.cwd(); // Reserved for potential cleanup
...
# Regular content`,
      ...
      for (const file of mixedFiles) {
        // Place non-test files in a regular directory, test files in tests/fixtures
        const dir = file.path === 'non-test-file.md' ? 'src' : 'tests/fixtures';
    ...
    it('should handle migration rollback correctly', async () => {
      const testFile = path.join(tempDir, 'tests/fixtures/rollback-test.md');
...
# Unicode Content 中文`,
      ...
      for (const testCase of edgeCases) {
        const filePath = path.join(tempDir, 'tests/fixtures', testCase.name);
    ...
    it('should preserve file permissions and timestamps', async () => {
      const testFile = path.join(tempDir, 'tests/fixtures/permissions-test.md');
      const content = `---
...
# Content`;
      ...
      expect(() => addTestMetadata(content, '/tests/large.md')).not.toThrow();
      expect(() => removeTestMetadata(content)).not.toThrow();
      
      const result = addTestMetadata(content, '/tests/large.md');
      expect(result).toContain('_dollhouseMCPTest: true');
    });

    it('should handle special characters in file paths', async () => {
      const specialPaths = [
        '/tests/files with spaces/test-file.md',
        '/tests/files-with-hyphens/test-file.md',
        '/tests/files_with_underscores/test-file.md',
        '/tests/files.with.dots/test-file.md',
        '/tests/files(with)parentheses/test-file.md'
      ];
