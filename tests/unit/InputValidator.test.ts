...
describe('InputValidator - Security Edge Cases', () => {
  ...
  describe('Combined Attack Vectors', () => {
    ...
    it('should handle homograph attacks', () => {
      ...
      homographAttacks.forEach(attack => {
        ...
        try {
          ...
          throw new Error(`Should have thrown for homograph attack: ${attack}`);
        } catch (error) {
    ...
    it('should resist timing attacks on validation', () => {
      ...
      for (let run = 0; run < testRuns; run++) {
        ...
        for (let i = 0; i < 100; i++) {
          ...
          try { validateFilename(validInput); } catch {
            // Intentionally empty - timing test, we only care about duration
          }
      ...
      // Test passes if at least 2 out of 5 runs succeed
      // Relaxed threshold to avoid flakiness in loaded environments
