...
describe('GitHubClient', () => {
  ...
  beforeEach(() => {
    ...
    githubClient = new GitHubClient(mockApiCache, rateLimitTracker, mockTokenManager as any);
  });
  ...
  describe('fetchFromGitHub', () => {
    ...
    it('should include GitHub token when available', async () => {
      ...
      // Configure mock TokenManager to return the token
      mockTokenManager.getGitHubTokenAsync.mockResolvedValue(validToken);
      ...
      expect(mockTokenManager.getGitHubTokenAsync).toHaveBeenCalled();
    });
