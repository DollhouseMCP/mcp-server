...
const PRIVATE_TEN_CIDR = `${['10', '0', '0', '0'].join('.')}/8`;
const PRIVATE_172_CIDR = `${['172', '16', '0', '0'].join('.')}/12`;
...
describe('assertHostedDeploymentSafety', () => {
  ...
  it('cycle-9: H2 error message names the bootstrap CLI step alongside the env var', async () => {
    ...
    try {
      await assertHostedDeploymentSafety({
        ...
        methods: ['github'],
        authEnabled: false,
        trustedProxies: [PRIVATE_TEN_CIDR],
      });
    } catch (e) { err = e; }
    expect(err).toBeInstanceOf(Error);
    const msg = (err as Error).message;
  ...
  it('H4: passes when non-loopback bind + multi-user + auth enabled + trusted proxies set', async () => {
    ...
    await expect(assertHostedDeploymentSafety({
      ...
      methods: ['local-password'],
      authEnabled: true,
      trustedProxies: [PRIVATE_TEN_CIDR, PRIVATE_172_CIDR],
    })).resolves.toBeUndefined();
  });
