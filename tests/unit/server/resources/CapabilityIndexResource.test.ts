...
// Mock the logger using unstable_mockModule for ES modules
...
describe('CapabilityIndexResource', () => {
  ...
  let fileOperationsService: InstanceType<typeof FileOperationsService>;
  ...
  beforeEach(async () => {
    ...
    // Create isolated temp portfolio directory
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'capability-index-test-'));
    capabilityIndexPath = path.join(tempDir, 'capability-index.yaml');

    // Point portfolio dir to temp location
    ...
    // Create resource instance with DI
    resource = new CapabilityIndexResource(fileOperationsService);
  });
  ...
  describe('Constructor and Initialization', () => {
    it('should initialize with correct path respecting DOLLHOUSE_PORTFOLIO_DIR', async () => {
      // Since beforeEach sets DOLLHOUSE_PORTFOLIO_DIR to tempDir,
      // the constructor should use that instead of the default ~/.dollhouse/portfolio
      const expectedPath = path.join(tempDir, 'capability-index.yaml');
  ...
  describe('loadCapabilityIndex()', () => {
    ...
    it('should log success message with element count', async () => {
      ...
      // Call loadCapabilityIndex directly to trigger logging
      await (resource as any).loadCapabilityIndex();
