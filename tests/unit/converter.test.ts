...
`;
        ...
        it('should handle large files for ZIP size limit validation', async () => {
            // This test validates the ZIP creation pipeline handles large files correctly.
            // Note: 50MB of repetitive data compresses extremely well (~few KB), so this
            // doesn't actually test the 100MB rejection. The size limit is enforced in
            // the converter implementation, not tested here due to compression efficiency.
            // Extended timeout for Windows CI runners where disk I/O is slower (Issue #341)
            const skillDir = createTestSkillDirectory('large-skill');
            const zipPath = path.join(testZipDir, 'large-skill.zip');
            createdFiles.push(zipPath, skillDir);

            // Create a large file (50MB) - compresses well due to repetitive content
            ...
            expect(fs.existsSync(zipPath)).toBe(true);
            // Compressed size should be much smaller than source due to repetitive content
            expect(stats.size).toBeLessThan(50 * 1024 * 1024);
        }, 30000); // Extended timeout for large file I/O on slower CI runners (Windows)

        it('should create ZIP with accurate size for incompressible data', async () => {
            // This test verifies that incompressible (random) data maintains its size
            // through the ZIP process, enabling proper size limit enforcement.
            // The actual 100MB limit is enforced in src/cli/convert.ts:extractZipFile()
            // which validates ZIP size before extraction (lines 142-147).
            const skillDir = createTestSkillDirectory('random-data-skill');
            const zipPath = path.join(testZipDir, 'random-data.zip');
            createdFiles.push(zipPath, skillDir);

            // Create a file with random (incompressible) data - 1MB is enough to verify
            const randomFile = path.join(skillDir, 'random-data.bin');
            ...
            // Random data should not compress significantly (ZIP overhead + ~same size)
            // This verifies size limit checks would work on real incompressible data
            expect(stats.size).toBeGreaterThan(900 * 1024); // At least 900KB
            expect(stats.size).toBeLessThan(1.5 * 1024 * 1024); // Under 1.5MB with overhead
        });

        it('should enforce 100MB ZIP size limit (validation logic test)', () => {
            // This test verifies the size limit validation logic matches convert.ts
            // The actual enforcement is in src/cli/convert.ts:extractZipFile() lines 142-147
            // We replicate the exact validation logic here to ensure correctness
            const MAX_ZIP_SIZE_BYTES = 100 * 1024 * 1024; // 100MB - must match convert.ts:52

            // Helper that replicates the validation logic from convert.ts
            const validateZipSize = (zipSize: number): void => {
                if (zipSize > MAX_ZIP_SIZE_BYTES) {
                    throw new Error(
                        `ZIP file too large: ${zipSize} bytes. Maximum allowed: ${MAX_ZIP_SIZE_BYTES} bytes.`
                    );
                }
            };

            // Test: Files under limit should pass
            expect(() => validateZipSize(50 * 1024 * 1024)).not.toThrow(); // 50MB
            expect(() => validateZipSize(99 * 1024 * 1024)).not.toThrow(); // 99MB
            expect(() => validateZipSize(100 * 1024 * 1024)).not.toThrow(); // Exactly 100MB

            // Test: Files over limit should be rejected
            expect(() => validateZipSize(100 * 1024 * 1024 + 1)).toThrow(/ZIP file too large/);
            expect(() => validateZipSize(150 * 1024 * 1024)).toThrow(/ZIP file too large/);
            expect(() => validateZipSize(500 * 1024 * 1024)).toThrow(/ZIP file too large/);
        });
