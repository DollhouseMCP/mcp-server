...
describe('Roundtrip Workflow E2E Tests', () => {
  ...
  beforeAll(async () => {
    ...
    // Note: Config save/restore removed - ConfigManager doesn't support full config updates
    // Tests are currently skipped (see Issue #598), so this is not needed
    ctx.originalConfig = null;
  ...
  afterAll(async () => {
    ...
    // Note: Config restore removed - no longer saving config in beforeAll
    // (ConfigManager doesn't support full config updates)
  ...
  describe('Phase 1: Collection Browsing and Installation', () => {
    ...
    test('should install content from collection', async () => {
      const result = await ctx.server.handleTool('install_collection_content', {
    ...
    test('should verify installed content exists locally', async () => {
      ...
      const installedSkill = result.data.elements.find(
  ...
  describe('Phase 2: Local Content Modification', () => {
    ...
    test('should verify local modifications were saved', async () => {
      const result = await ctx.server.handleTool('get_element_details', {
        name: ctx.testSkillName,
        type: 'skills'
      });
      ...
      expect(result.data.element.version).toBe('1.0.2');
    });
  });
  ...
  describe('Phase 3: Portfolio Management', () => {
    ...
    test('should submit content to portfolio without collection issue', async () => {
      const result = await ctx.server.handleTool('submit_collection_content', {
  ...
  describe('Phase 7: Complete Workflow Validation', () => {
    test('should validate complete roundtrip workflow integrity', async () => {
      ...
      const installResult = await ctx.server.handleTool('install_collection_content', {
        path: 'library/skills/roundtrip-test-skill.md'
      });
      ...
      if (installResult.success) {
        ...
        const submitResult = await ctx.server.handleTool('submit_collection_content', {
          content: ctx.testSkillName
        });
        ...
        const verifyResult = await ctx.server.handleTool('get_element_details', {
          name: ctx.testSkillName,
          type: 'skills'
        });

        expect(verifyResult.success).toBe(true);
        expect(verifyResult.data.element).toBeDefined();
        expect(verifyResult.data.element.version).toBe('1.0.4');
      }
    });
  });
});
...
if (!hasTestCredentials() && !process.env.CI) {
  describe.skip = describe;
  console.warn(getTestSkipMessage());
}
