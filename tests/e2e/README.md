...
cp tests/e2e/.env.test.local.example tests/e2e/.env.test.local

# Edit the file with your token
nano tests/e2e/.env.test.local
```
...
npx jest tests/e2e/real-github-integration.test.ts --verbose
```
...
npx jest tests/e2e/real-github-integration.test.ts -t "Single Element Upload"

# Error code validation only
npx jest tests/e2e/real-github-integration.test.ts -t "Error Code Validation"

# MCP tool flow tests
npx jest tests/e2e/mcp-tool-flow.test.ts --verbose
```
...
```bash
# Run cleanup utility
npx tsx tests/e2e/cleanup-test-data.ts
...
2. Review session notes in `docs/developer-guide/`
