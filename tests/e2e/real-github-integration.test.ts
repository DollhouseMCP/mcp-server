...
} from '../helpers/test-persona-factory.js';
...
describeOrSkip('Real GitHub Portfolio Integration Tests', () => {
  ...
  beforeAll(async () => {
    ...
    // Initialize portfolio manager with real TokenManager and JUST the repo name (not "owner/repo")
    const repoName = testEnv.testRepo.split('/')[1];
    portfolioManager = createRealPortfolioRepoManager(undefined, repoName);
  ...
  describe('Error Code Validation', () => {
    it('should return PORTFOLIO_SYNC_001 for invalid token', async () => {
      ...
      const badManager = createRealPortfolioRepoManager();
