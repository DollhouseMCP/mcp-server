...
// Test directory setup - use actual tmpdir for test directory creation
// Variables will be initialized per-test in beforeEach
let TEST_HOME: string;
let TEST_TELEMETRY_DIR: string;
let TEST_TELEMETRY_ID_PATH: string;
let TEST_TELEMETRY_LOG_PATH: string;
...
describe('Telemetry Integration Tests', () => {
  beforeEach(async () => {
    ...
    TEST_HOME = path.join(os.tmpdir(), 'dollhouse-telemetry-test-' + Date.now());
    TEST_TELEMETRY_DIR = path.join(TEST_HOME, '.dollhouse');
    TEST_TELEMETRY_ID_PATH = path.join(TEST_TELEMETRY_DIR, '.telemetry-id');
    TEST_TELEMETRY_LOG_PATH = path.join(TEST_TELEMETRY_DIR, 'telemetry.log');
    ...
    // Override home directory for telemetry using DOLLHOUSE_HOME_DIR
    // This follows the same pattern as DOLLHOUSE_PORTFOLIO_DIR
    process.env.DOLLHOUSE_HOME_DIR = TEST_HOME;

    // Enable telemetry explicitly for tests (default is opt-in/off)
    process.env.DOLLHOUSE_TELEMETRY = 'true';

    // Create test directory structure
    await fs.mkdir(TEST_TELEMETRY_DIR, { recursive: true });

    // Create file operations service for telemetry
    const fileLockManager = new FileLockManager();
    fileOperations = new FileOperationsService(fileLockManager);

    // Create telemetry instance
    telemetry = new OperationalTelemetry(fileOperations);
  });
  ...
  describe('Subsequent Run Scenario', () => {
    it('should reuse existing UUID on subsequent runs', async () => {
      ...
      // Create new telemetry instance for fresh state
      telemetry = new OperationalTelemetry(fileOperations);
  ...
  describe('Opt-Out Scenario', () => {
    it('should NOT create files when telemetry env is unset (default opt-in requirement)', async () => {
      delete process.env.DOLLHOUSE_TELEMETRY;
