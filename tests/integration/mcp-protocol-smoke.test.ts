...
const ELEMENT_TYPES: ElementTypeConfig[] = [
  ...
  },
  {
    ...
    createParams: {
      ...
      instructions: 'You are a workflow automation agent. Execute tasks methodically and report results.',
...
describe('MCP Protocol Smoke Tests', () => {
  ...
  for (const config of ELEMENT_TYPES) {
    describe(`${config.type} — Full Lifecycle`, () => {
      ...
      it(`should create a ${config.type}`, async () => {
        ...
        expect(response).not.toMatch(/failed|error|❌/i);
      }, TEST_TIMEOUT);
  ...
  describe('Concurrent Element Creation — Multiple Types', () => {
    it('should handle parallel creates across different element types', async () => {
      const elements = [
        { type: 'persona', name: 'smoke-concurrent-persona', params: { description: 'Concurrent persona', instructions: 'You are a concurrent test persona. Respond helpfully and accurately.' } },
      ...
      for (const result of results) {
        expect(result).toMatch(/created|success/i);
        expect(result).not.toMatch(/failed|error|❌/i);
      }
