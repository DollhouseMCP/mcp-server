...
sync:
  ...
  bulk:
    download_enabled: true`;
      ...
      expect(parsed.content).toBe(content); // Should return original content
...
sync:
  ...
  bulk:
    download_enabled: true`;
      ...
      expect(parsed).toHaveProperty('user.username', 'test-user');
      expect(parsed).toHaveProperty('user.email', 'user@example.com');
...
- Suggest character improvements`;
      ...
      expect(parsed.data.name).toBe('Creative Writer');
      expect(parsed.data.description).toBe('A creative writing assistant');
      expect(parsed.data.author).toBe('dollhousemcp');
      expect(parsed.data.tags).toEqual(['creative', 'writing', 'storytelling']);
...
danger: !!python/object/apply:os.system ['rm -rf /']`;
      ...
      // JSON_SCHEMA (and FAILSAFE_SCHEMA) should throw on unknown tags like !!js/function
      expect(() => {
        yaml.load(maliciousYaml, { schema: yaml.JSON_SCHEMA });
      }).toThrow();
    });
