...
describe('README Builder Integration Tests', () => {
  ...
  beforeEach(() => {
    // Store current working directory before each test
    testCwd = process.cwd();
  });

  afterEach(() => {
    // Restore working directory after each test
    // This ensures that if a test changes cwd and fails, it doesn't pollute other tests
    try {
      process.chdir(testCwd);
    } catch (_error) {
      // If testCwd doesn't exist, restore to original
    ...
    test('should validate config and report errors', async () => {
      ...
      try {
        ...
        try {
          ...
          expect(error.stdout).toContain('Invalid config');
        }
    ...
    test('should provide helpful error for missing config file', async () => {
      ...
      try {
        ...
        try {
          ...
          expect(error.stdout).toContain('Configuration file not found');
        }
      } finally {
        // Restore config
