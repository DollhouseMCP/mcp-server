...
import { createRealPortfolioRepoManager } from '../helpers/di-mocks.js';
...
describe('Portfolio Single Element Upload - GitHub API Response Fix', () => {
  let portfolioManager: InstanceType<typeof PortfolioRepoManager>;
  let originalFetch: typeof fetch;
  let container: InstanceType<typeof DollhouseContainer>;

  // Use test credentials from centralized config
  // GITHUB_TEST_REPOSITORY is now just the repo name (not "owner/repo")
  const expectedRepo = process.env.GITHUB_TEST_REPOSITORY || 'dollhouse-test-sandbox';
  const expectedUser = process.env.GITHUB_TEST_USERNAME || 'testuser';
  ...
  describe('GitHub API Response Handling', () => {
    it('should handle response with commit.html_url (standard case)', async () => {
      ...
      mockFetch.mockResolvedValueOnce({
        ...
        json: async () => ({
          content: {
            ...
            html_url: `https://github.com/${expectedUser}/${expectedRepo}/blob/main/personas/test.md`,
          },
          commit: {
            sha: 'abc123',
            html_url: `https://github.com/${expectedUser}/${expectedRepo}/commit/abc123`
          }
        })
      } as Response);
      ...
      expect(result).toBe(`https://github.com/${expectedUser}/${expectedRepo}/commit/abc123`);
    });
    ...
    it('should handle response with null commit (the bug from QA report)', async () => {
      ...
      expect(result).toBe(`https://github.com/${expectedUser}/${expectedRepo}/blob/main/personas/test.md`);
    });
    ...
    it('should use ultimate fallback when response has no useful data', async () => {
      ...
      expect(result).toBe(`https://github.com/${expectedUser}/${expectedRepo}/tree/main/skills`);
    });
  });
  ...
  describe('Single Element Upload Behavior', () => {
          ...
          return {
            ...
            json: async () => ({
              content: { 
                ...
                html_url: `https://github.com/${expectedUser}/${expectedRepo}/blob/main/personas/ziggy.md`
              },
              commit: { 
                html_url: `https://github.com/${expectedUser}/${expectedRepo}/commit/abc`
              }
            })
          } as Response;
        }
    ...
    it('simulates real user flow: upload Ziggy persona to personal GitHub portfolio', async () => {
      ...
      // Verify success
      expect(result).toContain(`github.com/${expectedUser}/${expectedRepo}`);
