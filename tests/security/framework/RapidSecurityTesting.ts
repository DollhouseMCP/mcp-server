...
export class RapidSecurityTesting {
  ...
  constructor() {
    ...
    this.server = new DollhouseMCPServer(container);
  }
...
`;
  ...
  private async testCommandInjection(): Promise<SecurityTestResult> {
    ...
    try {
      ...
      for (const payload of criticalPayloads) {
        ...
        const result = await this.server.createElement({
          name: payload,
          type: 'persona',
          description: 'test',
  ...
  private async testTokenExposure(): Promise<SecurityTestResult> {
    ...
    try {
      ...
      const fakeToken = 'ghp_1234567890abcdefghijklmnopqrstuvwxyz'; // NOSONAR - Intentional fake token for security testing
