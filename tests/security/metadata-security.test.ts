...
import type { DefaultElementProvider } from '../../src/portfolio/DefaultElementProvider.js';

describe('Metadata Detection Security Tests', () => {
  let tempDir: string;
  let provider: InstanceType<typeof DefaultElementProvider>;
  let DefaultElementProviderClass: typeof DefaultElementProvider;

  beforeEach(async () => {
    const { DefaultElementProvider: ProviderModule } = await import('../../src/portfolio/DefaultElementProvider.js');
    DefaultElementProviderClass = ProviderModule;

    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'metadata-security-test-'));

    provider = new DefaultElementProviderClass({
      loadTestData: false,
      useDefaultPaths: false,
      customDataPaths: [tempDir]
    });
  });
  ...
  afterEach(async () => {
    ...
    DefaultElementProviderClass.cleanup();
  });
