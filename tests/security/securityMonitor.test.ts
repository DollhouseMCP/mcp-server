...
const RUNTIME_REQUEST_SOURCE = 'runtime-request';
...
describe('SecurityMonitor', () => {
  ...
  describe('logSecurityEvent', () => {
    it('replays startup events when the unified listener requests the backlog', () => {
      ...
      expect(listener.mock.calls[1]?.[0]).toMatchObject({ source: RUNTIME_REQUEST_SOURCE });
  ...
  describe('clearOldEvents', () => {
    it('should remove events older than specified days', () => {
      ...
      SecurityMonitor['events'].push(oldEvent);
