...
function setupGlobalTestEnvironment(): void {
  ...
  // Clear static state before each test to prevent pollution
  // SecurityMonitor uses static state to track security events across the application
  // DefaultElementProvider has static caches that accumulate across tests
  // CollectionSeeder caches seed data that can pollute tests
  // FileDiscoveryUtil has a 5-second TTL cache that can cause test pollution
  // InstallationDetector caches the installation type that must be cleared per test
  // Without this cleanup, state accumulates and can cause test failures
  beforeEach(() => {
    SecurityMonitor.clearAllEventsForTesting();
    DefaultElementProvider.cleanup();
    CollectionSeeder.clearCache();
    FileDiscoveryUtil.clearCache();
