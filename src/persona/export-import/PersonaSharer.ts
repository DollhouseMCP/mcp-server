...
export class PersonaSharer {
  ...
  async importFromUrl(url: string): Promise<{ success: boolean; data?: any; message: string }> {
    try {
      ...
      if (!this.validateShareUrl(url)) {
        throw ErrorHandler.createError('Invalid or potentially malicious URL', ErrorCategory.VALIDATION_ERROR);
      }
      ...
      try {
        ...
        if (!response.ok) {
          throw ErrorHandler.createError(`Request failed with status ${response.status}`, ErrorCategory.NETWORK_ERROR);
        }

        // Validate Content-Type header
        const contentType = response.headers.get('content-type');
        if (!contentType || !contentType.toLowerCase().includes('application/json')) {
          throw ErrorHandler.createError('Invalid response type: expected JSON', ErrorCategory.NETWORK_ERROR);
        }
        ...
        if (contentLength && parseInt(contentLength) > maxSize) {
          throw ErrorHandler.createError('Response too large', ErrorCategory.VALIDATION_ERROR);
        }
  ...
  private async createGist(personaName: string, data: any): Promise<{ success: boolean; url?: string; gistId?: string }> {
    try {
      ...
      try {
        ...
        if (!response.ok) {
          throw ErrorHandler.createError(`GitHub API error: ${response.status}`, ErrorCategory.NETWORK_ERROR);
        }

        // Validate Content-Type for GitHub API response
        const contentType = response.headers.get('content-type');
        if (!contentType || !contentType.toLowerCase().includes('application/json')) {
          throw ErrorHandler.createError('Invalid GitHub API response type', ErrorCategory.NETWORK_ERROR);
        }
  ...
  private async importFromGist(gistId: string): Promise<{ success: boolean; data?: any; message: string }> {
    try {
      ...
      const rateLimitStatus = this.githubRateLimiter.checkLimit();
      if (!rateLimitStatus.allowed) {
        throw ErrorHandler.createError(`GitHub API rate limit exceeded. Please try again in ${Math.ceil(rateLimitStatus.retryAfterMs! / 1000)} seconds`, ErrorCategory.NETWORK_ERROR);
      }
      ...
      if (!this.validateShareUrl(gistUrl)) {
        throw ErrorHandler.createError('Invalid GitHub API URL', ErrorCategory.VALIDATION_ERROR);
      }
      ...
      try {
        ...
        if (!response.ok) {
          throw ErrorHandler.createError(`Failed to fetch gist: ${response.status}`, ErrorCategory.NETWORK_ERROR);
        }
        ...
        if (!personaFile) {
          throw ErrorHandler.createError('No persona data found in gist', ErrorCategory.VALIDATION_ERROR);
        }
  ...
  private importFromBase64Url(url: string): { success: boolean; data?: any; message: string } {
    try {
      ...
      if (!match) {
        throw ErrorHandler.createError('Invalid share URL format', ErrorCategory.VALIDATION_ERROR);
      }
