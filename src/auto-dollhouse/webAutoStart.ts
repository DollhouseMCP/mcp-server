...
export async function startPermissionServer(
  ...
  try {
    ...
    // Wire page infrastructure if available (Drawing Room feature — auto-dollhouse only).
    // Uses dynamic property checks to avoid compile errors when page infra isn't present.
    const anyResult = result as Record<string, unknown> | undefined;
    if (anyResult?.pageEventDispatcher && anyResult?.pageUpdateBroadcast
        && typeof (mcpAqlHandler as any).setPageInfrastructure === 'function') {
      (mcpAqlHandler as any).setPageInfrastructure(anyResult.pageEventDispatcher, anyResult.pageUpdateBroadcast);
