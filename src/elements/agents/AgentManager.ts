  ...
  */
  ...
  override async list(options?: { includePublic?: boolean }): Promise<Agent[]> {
    ...
    for (const agent of agents) {
      ...
      if (
        shouldBeActive &&
        (typeof agent.getStatus !== 'function' || agent.getStatus() !== ElementStatus.ACTIVE)
      ) {
  ...
  private synchronizeRecoveryState(
    ...
    try {
      ...
      SecurityMonitor.logSecurityEvent({
        ...
        details: 'Durable recovery succeeded but live agent synchronization failed',
        additionalData: { agentName: sourceAgent.metadata.name, completedGoalId },
      });
    }
