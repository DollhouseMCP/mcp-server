 ...
 *
 * Parses IPv4/IPv6 address strings into fixed-width byte form before classifying
 * them as public or non-public, so that no textual variant of an internal address
 * (hex/dotted IPv4-mapped IPv6, IPv4-compatible, NAT64, 6to4) can be misread as
 * public. Unparseable input always classifies as non-public (fail closed).
 *
...
/**
 * Parse an IP address string into canonical byte form.
 * Returns null for anything that is not a plain IPv4 dotted-quad or a valid
 * IPv6 textual form (zone identifiers are rejected).
 */
...
/**
 * Classify an IP address string as publicly routable.
 * Non-public covers loopback, unspecified, RFC1918, CGNAT, link-local,
 * multicast, ULA, documentation ranges, and every IPv6 form embedding a
 * non-public IPv4 address. Unparseable input is non-public.
 */
...
/** Replace a trailing embedded dotted-quad (e.g. `::ffff:1.2.3.4`) with its two hextets. */
...
/**
 * Extract the IPv4 address embedded in transition-mechanism IPv6 forms:
 * IPv4-mapped (::ffff:0:0/96), IPv4-compatible (::/96 — also subsumes the
 * unspecified :: and loopback ::1, which map to non-public 0.0.0.0/0.0.0.1),
