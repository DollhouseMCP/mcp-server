...
export class TokenManager {
   ...
   */
  private getTokenValidationLimiter(): RateLimiter {
  ...
  validateTokenFormat(token: string): boolean {
    ...
    return Object.values(TokenManager.GITHUB_TOKEN_PATTERNS).some(pattern =>
      pattern.test(token)
    );
  }
   ...
   */
  async getGitHubTokenAsync(): Promise<string | null> {
