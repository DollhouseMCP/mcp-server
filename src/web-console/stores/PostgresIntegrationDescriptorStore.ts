...
export class PostgresIntegrationDescriptorStore implements IIntegrationDescriptorStore {
  ...
  async findVisibleByProvider(
    ...
    const rows = await withSystemContext(this.db, tx =>
      tx.select().from(integrationProviderDescriptors).where(and(
        ...
        // Curated strictly wins over a same-provider BYO descriptor ('byo' < 'curated'
    ...
    const descriptor = rows.at(0);
    return descriptor ? fromDescriptorRow(descriptor) : null;
  }
  ...
  async upsert(
    ...
    const rows = await withSystemContext(this.db, async tx => {
      const locked = await tx.select().from(integrationProviderDescriptors)
        .where(descriptorIdentity(input)).for('update').limit(1);
      let existing = locked.at(0);

      if (!existing) {
        const inserted = await tx.insert(integrationProviderDescriptors)
          .values(toDescriptorValues(input, input.createdAt))
          .onConflictDoNothing()
          .returning();
        if (inserted.length > 0) return inserted;

        // Another transaction inserted the same logical descriptor after our
        // initial empty read. Observe and lock the winner before comparing
        // routing fingerprints so invalidation cannot be skipped.
        const raced = await tx.select().from(integrationProviderDescriptors)
          .where(descriptorIdentity(input)).for('update').limit(1);
        existing = raced.at(0);
        if (!existing) {
          throw new Error('Concurrent integration descriptor upsert did not leave a visible row');
        }
      }

      const current = fromDescriptorRow(existing);
      ...
      return tx.update(integrationProviderDescriptors)
        ...
        .where(eq(integrationProviderDescriptors.id, existing.id))
        .returning();
    });
    const descriptor = rows.at(0);
    if (!descriptor) throw new Error('PostgreSQL did not return integration descriptor row');
    return fromDescriptorRow(descriptor);
  }
}
