...
function typeFilterChip(key, label, singular, count) {
  return `<button class="type-filter${state.type === key ? ' active' : ''}" data-key="${key}"
...
function renderTypeFilters() {
  ...
  const chips = [typeFilterChip('all', 'All', '', items.length)];
  for (const t of TYPES) chips.push(typeFilterChip(t, TYPE_META[t].label, TYPE_META[t].singular, counts[t] ?? 0));
