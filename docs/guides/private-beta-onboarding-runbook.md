...
Enrollment treats optional provider email as unverified metadata. Ordinary `GithubSocialMethod` has a pre-existing requirement for a verified primary GitHub email; current enrollment acceptance does not remove it. Explicitly qualify the missing-primary-email adverse case and document the resulting ordinary-login limitation. Do not silently change that policy or describe fake verified-email/provider tests as proof of support for every GitHub account.
