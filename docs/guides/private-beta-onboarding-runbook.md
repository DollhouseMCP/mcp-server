...
Follow [#2684](https://github.com/DollhouseMCP/mcp-server/issues/2684) and [#2677](https://github.com/DollhouseMCP/mcp-server/issues/2677) for evolving qualification evidence, including unit/build/lint results, PostgreSQL composition and ordinary-login journeys, and the built-browser journey across two replicas. These suites use fake SMTP and GitHub providers; they do not prove live mail receipt, provider configuration, real GitHub authentication, or deployment readiness. Requalify the exact release candidate and record its source SHA, artifact and test results instead of treating an earlier local candidate as release evidence.
...
Every private-beta tester must use a GitHub account with a verified primary email available to the authentication OAuth app. This is the accepted eligibility prerequisite for the qualified cohort: ordinary `GithubSocialMethod` retains its existing verified-primary-email requirement. Confirm eligibility before issuing an invitation and qualify the missing-primary-email rejection separately. Enrollment still treats optional provider email as unverified metadata; the GitHub email may differ from the invitation email and must never trigger an email-based account merge. [#2743](https://github.com/DollhouseMCP/mcp-server/issues/2743) remains open for eligibility messaging and usability follow-up, not as an enablement blocker for this explicitly qualified cohort. Authentication policy is unchanged; fake verified-email/provider tests do not prove support for every GitHub account.
