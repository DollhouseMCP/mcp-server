...
Memory auto-loading is the most variable-duration phase of server startup. Understanding how it works helps you keep startup fast — especially with large portfolios.

### How Deferred Loading Works

As of v2.0, memory auto-loading runs **after** the MCP connection is established:

1. **Critical path** (~50-200ms): Config checks, migration, portfolio init, tool registration
2. **MCP connect**: Tools become available — clients can call them immediately
3. **Deferred setup** (1-3s typical): Memory auto-load, activation restore, background validator

If a tool call arrives before deferred setup completes, the server automatically buffers the request (up to 10 seconds) and releases it once memories are loaded. This prevents stale-data responses without requiring client-side retry logic.

### Diagnosing Slow Startups

Use `get_build_info` to inspect startup timing:

```
get_build_info
```

The `startup` section of the response shows:

| Field | Description |
|-------|-------------|
| `status` | `"ready"` when all setup is complete, `"initializing"` while deferred work is running |
| `deferredSetupComplete` | `true` once memories and activations are fully loaded |
| `startupTimingMs.criticalPathMs` | Time (ms) before MCP connect — this is time-to-first-tool-call |
| `startupTimingMs.deferredMs` | Time (ms) for post-connect work (memory loading, activations) |
| `startupTimingMs.totalMs` | Total startup time from process start |
| `startupTimingMs.phases` | Per-phase breakdown (e.g., `memory_autoload`, `activation_restore`) |

**Example output** (startup section):

```
Status: Ready
Deferred Setup: Complete
Critical Path: 130ms
Deferred Work: 1747ms
Total Startup: 1877ms
Phases:
  - config_checks: 38ms (critical)
  - config_manager: 92ms (critical)
  - memory_autoload: 1203ms (deferred)
  - activation_restore: 312ms (deferred)
  - log_hooks: 52ms (deferred)
  - danger_zone_init: 48ms (deferred)
  - pattern_encryption: 87ms (deferred)
  - background_validator: 45ms (deferred)
```

> **Note**: Conditional phases `migration` and `portfolio_init` appear only when a migration is needed or the portfolio directory doesn't exist yet. They run on the critical path.

If `memory_autoload` dominates the deferred phase, you have too many or too-large auto-load memories.

### Performance by Memory Count

| Auto-load memories | Typical deferred setup time | Guidance |
|--------------------|----------------------------|----------|
| 1-10 | < 500ms | No action needed |
| 10-50 | 500ms - 2s | Acceptable for most deployments |
| 50-100 | 2-5s | Review priorities; consider disabling low-value auto-loads |
| 100-500 | 5-15s | Reduce count aggressively; use explicit `memories` list in config |
| 500+ | 15s+ | Likely hitting the 10s request buffer timeout; split or disable |

### Recommendations for Large Portfolios

1. **Audit auto-load memories regularly**: List all auto-load memories and remove stale ones:
   ```
   search_portfolio type="memories" query="autoLoad"
   ```

2. **Use the explicit list**: Instead of relying on per-file `autoLoad: true` flags across hundreds of memories, use the config `memories` array to whitelist exactly what loads:
   ```yaml
   autoLoad:
     enabled: true
     memories:
...
3. **Tune priorities**: If many memories must auto-load, ensure the most important ones have low priority numbers (1-10) so they load first. Less critical context can use higher numbers and will load later in the sequence.

4. **Keep memories small**: The 1-5KB recommendation (see [Memory Size](#memory-size)) is especially important for auto-load memories. A 100KB memory takes ~10x longer to parse than a 5KB one.

5. **Monitor with `get_build_info`**: After making changes, call `get_build_info` to verify `memory_autoload` duration decreased.

### READY Signal for Bridge Clients

The server emits `DOLLHOUSEMCP_READY\n` to stderr immediately after MCP connect — before deferred setup completes. Bridge clients and process managers should watch for this sentinel rather than using a fixed delay:

```
# Bridge startup pseudocode
process = spawn("node", ["dist/index.js"])
wait_for_line(process.stderr, "DOLLHOUSEMCP_READY")
# Now safe to send MCP requests — they will be buffered until deferred setup completes
```

> **See also**: [Logging Configuration — High-Volume / Constrained Disk](logging.md#3-high-volume--constrained-disk) for guidance on large log directories, which can compound startup variability.
...
Memory auto-loading runs as **deferred setup** — after the MCP connection is established, not before. This means tools are available immediately on connect; memories finish loading 1-3 seconds later.
...
ConfigManager.initialize()
  ↓
MCP connect() — tools available
  ↓
DOLLHOUSEMCP_READY sentinel emitted
  ↓
Deferred setup begins (post-connect)
  ↓
...
> **Note**: If a tool call arrives before deferred setup completes, it is automatically held (up to 10 seconds) until memories are loaded. See [Startup Performance](#startup-performance) for details.
...
- [Public Beta Onboarding](public-beta-onboarding.md) - Getting started with all element types including memories
- [Environment Variables](environment-variables.md) - Server configuration options
- [Memory Injection Protection](../security/memory-injection-protection.md) - Memory security details
- [API Reference](../reference/api-reference.md) - Complete MCP tool reference
- [Logging Configuration](logging.md) - Log growth management for large deployments
