...
4. **Explicit Opt-In** - Disabled unless you set a single environment variable
...
> **Note:** Remote telemetry also requires `DOLLHOUSE_TELEMETRY=true` because the entire telemetry stack is opt-in by default.
...
1. **Do nothing** – leaving `DOLLHOUSE_TELEMETRY_OPTIN` unset keeps remote telemetry off
2. **Set `DOLLHOUSE_TELEMETRY_NO_REMOTE=true`** – Keeps local logs even if you opt into remote later
...
# Option 3: Opt-in to remote telemetry (requires DOLLHOUSE_TELEMETRY=true)
export DOLLHOUSE_TELEMETRY=true
export DOLLHOUSE_TELEMETRY_OPTIN=true
```
...
## Telemetry Controls (Opt-In)

Telemetry is **disabled by default**. Use the following environment variables to enable or disable the pieces you need:

1. **Enable local telemetry**: `DOLLHOUSE_TELEMETRY=true`
2. **Enable remote telemetry (in addition to #1)**: `DOLLHOUSE_TELEMETRY_OPTIN=true`
3. **Force local-only, even if remote is enabled**: `DOLLHOUSE_TELEMETRY_NO_REMOTE=true`
4. **Disable everything again**: unset `DOLLHOUSE_TELEMETRY` or set it to `false`

### Method 1: Environment Variables (Recommended)

**Bash/Zsh:**
```bash
# Enable local telemetry (add to ~/.bashrc or ~/.zshrc to persist)
export DOLLHOUSE_TELEMETRY=true

# Optional: enable remote telemetry (requires local telemetry to be enabled)
export DOLLHOUSE_TELEMETRY_OPTIN=true
...
set -x DOLLHOUSE_TELEMETRY true

# Optional remote telemetry
set -x DOLLHOUSE_TELEMETRY_OPTIN true
...
```powershell
# Enable (user-level)
[Environment]::SetEnvironmentVariable("DOLLHOUSE_TELEMETRY", "true", "User")

# Optional remote telemetry
[Environment]::SetEnvironmentVariable("DOLLHOUSE_TELEMETRY_OPTIN", "true", "User")
...
```cmd
REM Enable (user-level)
setx DOLLHOUSE_TELEMETRY true

REM Optional remote telemetry
setx DOLLHOUSE_TELEMETRY_OPTIN true
...
1. **Read the Code**
   ...
   - Location: `src/telemetry/OperationalTelemetry.ts` (initialized at server startup)
...
1. **All telemetry code is open source**
   ...
   - Location: `src/telemetry/OperationalTelemetry.ts`
...
- **How to enable**: `export DOLLHOUSE_TELEMETRY=true`
- **How to disable**: `export DOLLHOUSE_TELEMETRY=false`
...
5. ✅ **User control** - Explicit opt-in plus easy disable, inspection, deletion
