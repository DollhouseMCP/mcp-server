...
> **Quick setup?** See the [Quick Start Guide](quick-start.md) for platform-specific install instructions covering Claude Desktop, Claude Code, Cursor, Windsurf, VS Code, Cline, Continue, Gemini CLI, OpenAI Codex, Docker, and local LLMs.

This guide provides advanced configuration details beyond what the Quick Start covers.

## Overview

DollhouseMCP is a platform-agnostic MCP server that works with any client supporting stdio transport and JSON-RPC communication.

The fastest way to install is `npx` (no build required):
...
For development or custom setups, install locally and point to the built output:
- `npm install @dollhousemcp/mcp-server` then reference `node_modules/@dollhousemcp/mcp-server/dist/index.js`
- Or clone the repo, `npm install && npm run build`, then reference `dist/index.js`
...
```mermaid
graph LR
    A[Claude Desktop] --> B(claude_desktop_config.json);
    B --> C(DollhouseMCP Server);
```
...
```mermaid
graph LR
    A[VS Code] --> B(.mcp.json / ~/.claude.json);
    B --> C(DollhouseMCP Server);
```
...
- Confirm `dist/index.js` exists (run `npm run build` after pulling updates)
