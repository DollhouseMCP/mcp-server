...
│   └── RapidSecurityTesting.ts     # Fast CI/CD helpers
├── tests/
│   ├── command-injection.test.ts   # Command injection
│   ├── path-traversal.test.ts      # Filesystem traversal
│   ├── yaml-deserialization.test.ts # YAML safety (plus expanded variants)
│   ├── input-validation-security.test.ts
│   └── mcp-tools-security.test.ts
├── regression/                      # Regression suites (if enabled)
├── setup.ts                         # Shared setup hooks
└── index.ts                         # Entry point used by scripts
```
...
# This creates: tests/security/tests/new-tool-name-security.test.ts
```
