...
Both cookies are host-only (`__Host-`, no Domain), Path=/, Secure, HttpOnly, and SameSite=Lax. Lax permits the top-level GitHub OAuth callback. HTTPS is required; these helpers provide no HTTP relaxation. The session cookie Max-Age must be capped to its remaining server-record lifetime. The owner cookie is established before exchange with the original creation +168-hour absolute horizon (see `onboardingOwnerCookieMaxAgeSeconds`); it intentionally survives the 15-minute unclaimed server record. This preserves the browser binding if an atomic claim succeeds but its response is lost. Cookie possession never replaces live server-record validation. Clearing a cookie alone does not revoke a server record.
...
The PostgreSQL `exchangeClaim` entrypoint consumes the claim, extends the managed owner, and creates the restricted session in one transaction. It derives session references from the returned claim and validates live authority before atomically replacing the prior session with fresh session and CSRF hashes. A crash before commit rolls everything back, leaving the invitation unconsumed; a lost successful response can resume using the already-established owner cookie and same invitation credential. A failure must never set a cookie for an uncommitted session. Do not compose separate claim and session transactions for initial exchange.
