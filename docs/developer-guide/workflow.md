...
- Run `scripts/setup-gitflow-guardian.sh` after cloning to install local hooks that warn when you drift from the expected GitFlow. These are the same guardrails used by the automation agents.
- Record any necessary bypasses (for example, emergency hotfixes) in the current `docs/agent/development/SESSION_*` note so future agents understand why a hook was skipped. The archival helper will move it to `docs/session-history/` when it ages out.
- Persistent false positives—most commonly when branching from `develop`—should be documented and cross-referenced in the agent workspace (`docs/agent/README.md` points to the relevant historical notes).
...
### Before Creating PR
Run these checks in order (fail fast - catch critical issues early):

1. **Run pre-commit checks** (security tests and dependency audit):
   ```bash
   npm run pre-commit    # Runs security:rapid + npm audit --audit-level=high
   ```
2. **Run linter** (code style validation):
   ```bash
   npm run lint          # Check code style (must pass with 0 warnings)
...
3. **Run local tests**:
   ...
   ```
4. **Test Docker build** if changes affect containerization:
   ```bash
   docker build -f docker/Dockerfile -t dollhousemcp-test .
   ```
5. **Verify persona functionality** if changes affect core features
