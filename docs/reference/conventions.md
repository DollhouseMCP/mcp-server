...
Session notes stored in the repository (`docs/agent/development/`) use a different format optimized for visibility and version control.
...
## Retention Policy Conventions

The retention policy system (Issue #51) follows strict safety-first conventions to prevent accidental data loss.

### Default Configuration

**IMPORTANT**: Retention is DISABLED by default. This is intentional for data safety.

```yaml
retentionPolicy:
  enabled: false                    # Nothing auto-deleted
  enforcement_mode: disabled        # Safe default
```

### Enabling Retention

When enabling retention, always follow this progression:

1. **Start disabled** - Default state, safest option
2. **Test with manual** - Explicit control over cleanup
   ```yaml
   enabled: true
   enforcement_mode: manual
   ```
3. **Add safety controls** - Enable all protections
   ```yaml
   safety:
   ...
   ```
4. **Monitor in production** - Watch logs, verify behavior
5. **Only then consider automatic** - After thorough testing

### Safety Control Conventions

Always enable these safety controls in production:

- `require_confirmation: true` - User must approve deletions
- `dry_run_first: true` - Preview before executing
- `warn_on_expiring: true` - Advance warning system
- `log_deletions: true` - Audit trail required
- `backup_before_delete: true` - Recovery possible

### Enforcement Mode Guidelines

| Mode | When to Use | Who Uses |
|------|-------------|----------|
| `disabled` | Default, development, testing | Everyone initially |
| `manual` | Production with controlled cleanup | Most production users |
| `on_load` | Privacy-first auto-expiration | Signal-style use cases only |
| `scheduled` | Background maintenance | Future: high-volume systems |

### Configuration Naming

When overriding retention settings per-memory:
...
When creating memories with retention policies:

1. **Document the TTL**: Why this specific duration?
2. **Tag appropriately**: `retention-policy`, `auto-expire`
3. **Warn users**: If memory will be deleted automatically
4. **Provide recovery**: Backup strategy documented

### Per-Use-Case Conventions

**Legal/Compliance**:
- TTL: 7 years (2555 days) minimum
- Backups: 365 days minimum
- Audit: Full logging required
- Mode: Manual only

**Privacy/Security**:
- TTL: 7-30 days maximum
- Backups: Optional (often disabled for privacy)
- Audit: Minimal (privacy trade-off)
- Mode: `on_load` acceptable

**Storage Management**:
- TTL: 90-180 days typical
- Backups: 30 days
